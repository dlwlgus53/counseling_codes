...
PTSD_INPUT="${PTSD_INPUT:-${ROOT_DIR}/data/raw}"
DATA_PATH="${DATA_PATH:-${ROOT_DIR}/data/ptsd_ko_train.jsonl}"
OUTPUT_DIR="${OUTPUT_DIR:-${ROOT_DIR}/checkpoints/qwen3-vl-8b-ptsd-kor-lora}"
...
WANDB_PROJECT="${WANDB_PROJECT:-ptsd-qwenvl}"
WANDB_RUN_NAME="${WANDB_RUN_NAME:-qwen3-vl-8b-ptsd-kor-lora}"
...
"${PYTHON_BIN}" "${ROOT_DIR}/src/build_ptsd_ko_qwen_json.py" \
  --input "${PTSD_INPUT}" \
  --output "${DATA_PATH}" \
...
"${TORCH_LAUNCH[@]}" \
  ...
  "${ROOT_DIR}/src/train_ptsd_kor_lora.py" \
  --model_name_or_path "${MODEL_NAME_OR_PATH}" \
  --data_path "${DATA_PATH}" \
  --output_dir "${OUTPUT_DIR}" \
  --max_length "${MAX_LENGTH}" \
