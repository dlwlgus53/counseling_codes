#!/usr/bin/env python3
"""Text-only LoRA SFT for Qwen3-VL on Korean panic emergency counseling data."""
...
class PanicQwenDataset(Dataset):
...
class IOSampleLoggerCallback(TrainerCallback):
    ...
    def _build_record(self, model: Any, example: dict[str, Any], step: int, index: int) -> dict[str, Any]:
        messages = normalize_messages(example["messages"])
        prompt_messages = messages[:-1]
        prompt_text = self._messages_text(prompt_messages)
...
def print_batch_summary(args: argparse.Namespace, dataset_size: int, world_size: int) -> None:
    ...
    if not is_main_process():
        return
    print("==== PANIC Qwen3-VL Text-only LoRA Training ====", flush=True)
...
def main() -> None:
    ...
    train_dataset = PanicQwenDataset(args.data_path)
