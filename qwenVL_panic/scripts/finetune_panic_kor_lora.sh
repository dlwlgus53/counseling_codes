...
if [[ -f /home/jihyunlee/anaconda3/bin/activate ]]; then
  # The project runtime is expected to be the existing `mirror` conda env.
  source /home/jihyunlee/anaconda3/bin/activate mirror
fi

MODEL_NAME_OR_PATH="${MODEL_NAME_OR_PATH:-Qwen/Qwen3-VL-8B-Instruct}"
PANIC_JSON_PATH="${PANIC_JSON_PATH:-${ROOT_DIR}/data/raw}"
DATA_PATH="${DATA_PATH:-${ROOT_DIR}/data/panic_ko_train.jsonl}"
OUTPUT_DIR="${OUTPUT_DIR:-${ROOT_DIR}/checkpoints/qwen3-vl-8b-panic-kor-lora}"
...
WANDB_PROJECT="${WANDB_PROJECT:-panic-qwenvl}"
WANDB_RUN_NAME="${WANDB_RUN_NAME:-qwen3-vl-8b-panic-kor-lora}"
REPORT_TO="${REPORT_TO:-wandb}"

PYTHON_BIN="${PYTHON_BIN:-}"
if [[ -z "${PYTHON_BIN}" ]]; then
  MIRROR_ENV_PYTHON="/home/jihyunlee/anaconda3/envs/mirror/bin/python"
  if [[ -x "${MIRROR_ENV_PYTHON}" ]]; then
...
"${PYTHON_BIN}" "${ROOT_DIR}/src/build_panic_ko_qwen_json.py" \
  --input "${PANIC_JSON_PATH}" \
  --output "${DATA_PATH}" \
...
PANIC_JSON_PATH=${PANIC_JSON_PATH}
DATA_PATH=${DATA_PATH}
...
"${TORCH_LAUNCH[@]}" \
  ...
  "${ROOT_DIR}/src/train_panic_kor_lora.py" \
  --model_name_or_path "${MODEL_NAME_OR_PATH}" \
  --data_path "${DATA_PATH}" \
  --output_dir "${OUTPUT_DIR}" \
  --max_length "${MAX_LENGTH}" \
